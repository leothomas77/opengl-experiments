...
//--------------------------------------------------------------------------------------
// CFirstPersonAndOrbitCamera
//--------------------------------------------------------------------------------------
CFirstPersonAndOrbitCamera::CFirstPersonAndOrbitCamera()
    : CFirstPersonCamera()
{
    CFirstPersonCamera::SetRotateButtons(true, false, false, false);
}

VOID CFirstPersonAndOrbitCamera::FrameMove( FLOAT fElapsedTime )
{
    if( DXUTGetGlobalTimer()->IsStopped() ) {
        if (DXUTGetFPS() == 0.0f) fElapsedTime = 0;
        else fElapsedTime = 1.0f / DXUTGetFPS();
    ...
    // Get keyboard/mouse/gamepad input
    GetInput( m_bEnablePositionMovement, true, true, m_bResetCursorAfterMove );
    ...
    // Transform vectors based on camera's rotation matrix
    D3DXVECTOR3 vWorldLeft, vWorldUp, vWorldAhead;
    D3DXVECTOR3 vLocalLeft  = D3DXVECTOR3( 1, 0, 0 );
    D3DXVECTOR3 vLocalUp    = D3DXVECTOR3( 0, 1, 0 );
    D3DXVECTOR3 vLocalAhead = D3DXVECTOR3( 0, 0, 1 );
    D3DXVec3TransformCoord(&vWorldLeft,  &vLocalLeft,  &mCameraRot);
    D3DXVec3TransformCoord(&vWorldUp,    &vLocalUp,    &mCameraRot);
    D3DXVec3TransformCoord(&vWorldAhead, &vLocalAhead, &mCameraRot);
    ...
    D3DXVECTOR3 at2eye = m_vEye - m_vLookAt;
    float lookLen = D3DXVec3Length(&at2eye);
    ...
    // Update the lookAt position based on the eye position 
    at2eye = -vWorldAhead * lookLen;
    m_vLookAt = m_vEye - at2eye;

    // Orbit eye around look-at point
    if (m_nMouseWheelDelta) {
        float newLookLen = __max(1.f, lookLen - (m_fMoveScaler * m_nMouseWheelDelta)/1200.f);
        D3DXVec3Scale(&at2eye, &at2eye, newLookLen / lookLen);
        lookLen = newLookLen;
        m_nMouseWheelDelta = 0;
    }
    if (m_bMouseRButtonDown) {
        D3DXMATRIX R;
        D3DXMatrixRotationYawPitchRoll(&R, m_vRotVelocity.x, m_vRotVelocity.y, 0);

        D3DXVECTOR3 p;
        D3DXVec3TransformCoord(&p, &vLocalAhead, &R);
        D3DXVec3TransformCoord(&p, &p, &mCameraRot);
        D3DXVec3Scale(&at2eye, &p, -lookLen);
    }
    m_vEye = m_vLookAt + at2eye;
