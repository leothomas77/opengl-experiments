...
{
    SetFont( 0, L"Arial", 10, FW_NORMAL );
...
void CDXUTComboBox::Render( float fElapsedTime )
{
    ...
    if( !bSBInit )
    {
        ...
        if( m_pDialog->GetManager()->GetFontNode( pElement->iFont )->nHeight )
            m_ScrollBar.SetPageSize( RectHeight( m_rcDropdownText ) /
                                     (m_pDialog->GetManager()->GetFontNode(
                                             pElement->iFont )->nHeight + 10) );
        else
            m_ScrollBar.SetPageSize( RectHeight( m_rcDropdownText ) );
        bSBInit = true;
    }
    ...
    if( pFont )
    {
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < m_Items.GetSize(); i++ )
        {
            ...
            int yDelta = pFont->nHeight + 10;
            nRemainingHeight -= yDelta;
            ...
            SetRect( &pItem->rcActive, m_rcDropdownText.left, curY,
                     m_rcDropdownText.right, curY + yDelta );
            curY += yDelta;
