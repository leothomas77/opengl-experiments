                         ...
                         ID3D11DeviceContext *d3dDeviceContext)
{
    ...
    const size_t hairVertices = mHairFile.GetHeader().point_count;
    const size_t bufferSize = sizeof(Vertex) * hairVertices;
    ...
    mTransformedDepths = new TransformedDepth[hairVertices];
...
void
Hair::CreateBins(UINT numBins)
{
    mBins.resize(numBins);
    mInsertedIntoBins.resize(numBins);

    // Create bins
    for (UINT i = 0; i < numBins; ++i) {
        mBins[i].resize(mNumHairSegments); // max possible
        mInsertedIntoBins[i] = 0;
    }
}
...
Hair::FillDefaultIndices(ID3D11DeviceContext *d3dDeviceContext)
{
    ...
    assert(p == mHairFile.GetHeader().point_count);
                     ...
                     float x, float y, float z)
{
    ...
    for (size_t i = 0; i < hairVertices; ++i) {
        vertexBufferData[i].position[0] = *points -  0         + x - 50; ++points;
        vertexBufferData[i].position[2] = -(*points + 30) + 25 + z - 50; ++points;
        vertexBufferData[i].position[1] = *points - 8          + y - 50; ++points;
                  ...
                  const D3DXMATRIXA16 &cameraWorldView)
{
    ...
    const size_t hairVertices = mHairFile.GetHeader().point_count;
    const D3DXVECTOR3 *points = (const D3DXVECTOR3 *) mHairFile.GetPointsArray();

    // Transform hair vertices in camera space and save the depth
    for (UINT i = 0; i < hairVertices; ++i) {
        mTransformedDepths[i].depth = TransformZ(points[i], cameraWorldView);
    }
...
void
Hair::SortPerBin(ID3D11DeviceContext *d3dDeviceContext, 
                 const D3DXMATRIXA16 &cameraWorldView,
                 UINT numBins)
{
    mHairSorted = true;
    ...
    // Transform lines in camera space and find min/max z
    float minZ = std::numeric_limits<float>::max();
    float maxZ = std::numeric_limits<float>::min();
    for (UINT i = 0, p = 0; i < mNumHairSegments; ++i, p += 2) {
        float z1 = TransformZ(points[mIndices[p + 0]], cameraWorldView);
        float z2 = TransformZ(points[mIndices[p + 1]], cameraWorldView);
        // Use midpoint of line segment to as sort key
        mHairSegments[i].midPointDepth = (z1 + z2) / 2.0f;
        mHairSegments[i].i1 = mIndices[p + 0];
        mHairSegments[i].i2 = mIndices[p + 1];

        minZ = std::min(minZ, mHairSegments[i].midPointDepth);
        maxZ = std::max(maxZ, mHairSegments[i].midPointDepth);
    }

    for (UINT b = 0; b < numBins; ++b) {
        mInsertedIntoBins[b] = 0;
    }

    // Assign lines to bins
    float binSize = (maxZ - minZ) / numBins;
    for (UINT i = 0; i < mNumHairSegments; ++i) {
        UINT bin = (UINT) ((mHairSegments[i].midPointDepth - minZ) / binSize);
        bin = std::min(bin, numBins - 1); // clamp for the line furtherst out
        mBins[bin][mInsertedIntoBins[bin]] = i;
        ++mInsertedIntoBins[bin];
    }

    // Flatten based on binning
    HRESULT hr;
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    hr = d3dDeviceContext->Map(mHairIndexBuffer,
                               0, D3D11_MAP_WRITE_DISCARD, 0, 
                               &mappedResource);
    DWORD *indexBufferData =
        reinterpret_cast<DWORD*>(mappedResource.pData);
    UINT v = 0;
    for (UINT b = numBins - 1; b > 0; --b) {
        for (UINT i = 0; i < mInsertedIntoBins[b]; ++i) {
            indexBufferData[v++] = mHairSegments[mBins[b][i]].i1;
            indexBufferData[v++] = mHairSegments[mBins[b][i]].i2;
        }
    }
    assert(v == mNumHairSegments * 2);
    d3dDeviceContext->Unmap(mHairIndexBuffer, 0);
}
